...
Stanford's Minimum Security Standards for endpoint devices also require ANY phone or mobile device used to access High, Moderate, or Low-Risk Stanford information (including through email and calendar) to be protected with [Mobile Device Management](https://med.stanford.edu/irt/security/alldevices/mdm.html), whether it's your own personal phone or a Stanford-owned phone.
...
When conducting analyses, store data files only on the approved secure storage system, specify the secure file path in your integrated development environment, and access and analyze data from the secure location. Refer to @sec-file-paths for suggested R workflows implementing these practices.
